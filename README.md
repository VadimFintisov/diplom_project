...
На данном этапе необходимо учесть ограничение, что все три файла, участвующие в проверке – файл скрипта, файл с эталонным значением хэш-кода и файл подписи – должны находиться в одной директории. При этом название файла с эталонным значением хэш-кода должно быть такое же, как название файла скрипта, но с добавлением в конце «.hash», а название файла подписи должно быть такое же, как название файла с эталонным значением хэш-кода, но с добавлением в конце «.sig». Например, файл скрипта называется «script.py», значит, название файла с эталонным значением хэш-кода должно быть «script.py.hash», а файла с подписью – «script.py.hash.sig». При этом для защиты файлов от изменения администратору необходимо настроить разграничение прав доступа так, чтобы владельцем файла с эталонным значением хэш-кода и файла подписи было приложение FUSE и доступ был разрешен только на чтение. Всем другим субъектам системы доступ должен быть полностью закрыт.
...
/home/.keys# gpg --output key.pub --export YourName

    2. Монтирование виртуальной файловой системы FUSE и запуск скрипта на исполнение интерпретатором Python. Проверка системного журнала.
    Чтобы примонтировать виртуальную файловую систему FUSE, необходимо выбрать ранее созданную точку монтирования, которая должна быть пустой, и выполнить команду:
...
а в системном журнале при этом никаких записей не появляется. Это обусловлено тем, что проверка целостности пройдена успешно.
    ...
    3. Изменение файла скрипта. Запуск на исполнение интерпретатором Python. Проверка системного журнала.
    Теперь нужно изменить содержимое файла «good.py», например, дописав в него какую-либо информацию, тем самым будет нарушена целостность файла.После этого необходимо еще раз запустить на исполнение данный файл из примонтированной области
