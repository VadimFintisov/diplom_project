...
После подписи в этой же папке будут находиться файлы с хеш-суммой и цифровой подписью от оригинального скрипта – они будут использоваться для проверки подлинности файла test.py.
...
8)	Следующим шагом будет расчет контрольной суммы используемого скрипта с помощью утилиты «gostsum». Чтобы рассчитать контрольную сумму данной утилитой и сохранить значение в файл, для этого переходим в папку Домашняя/apps, Сервис->Запустить терминал, далее необходимо ввести команды:

`sudo su`

`gostsum *название файла* > *название выходного файла*`

например: `gostsum good.py > good.py.hash`
...
например: `gpg --sign good.py.hash`
...
например: `gpg -u Vadim --sign good.py.hash`
...
например: `gpg --export --output Output_file 5D38E46B9350FE5857AC5B95EC8B401CB1C237C3`
...
например: `gpg --import  *имя вашего открытого ключа в папке «pubring.gpg»*`
...
например: `gpg --decrypt good.py.hash > good_d.py`
...
например: `gostsum good_d.py > good_d.py.hash`
...
Ниже будет приведен контрольный пример:

1. Создание файла скрипта, подсчет и вывод его эталонного значения хэш-кода в файл, подписывание файла с эталонным значением хэш-кода валидным ключом.
для начала получаем права суперпользователя:

`sudo su`

затем выполняем команду:

/home/apps# `nano good.py`

куда вписываем любую команду на языке python, например: 

`print ("Hello, IT-Planet!")`

далее сохраняем изменения утверждаем имя файла и нажимаем ctrl+X.

После того, как файл создан, необходимо рассчитать его контрольную сумму и результат вывести в файл «good.py.hash» с помощью встроенной утилиты «gostsum», в данном случае мы рассматриваем стандартный вариант, где длина хэш-кода алгоритма, описанного в ГОСТ Р 34.11-2012 равна 256 битам.

/home/apps# `gostsum -o good.py.hash good.py`

Следующим шагом будет создание отделенной подписи файла «good.py.hash» с помощью утилиты «gpg».

home/apps# `gpg -b good.py.hash`

После этих действий в директории «/home/apps» будут находиться три файла:
    1. good.py
    2. good.py.hash
    3. good.py.hash.sig

Открытый ключ мастер-ключа, с помощью которого была сделана подпись, необходимо экспортировать в директорию «/home/.keys»:

/home/.keys# `gpg --output key.pub --export MyName`

где MyName - ID пользователя, которрый вы ввели на этапе создания ключа, в нашем случае это Vadim

2. Монтирование виртуальной файловой системы FUSE и запуск скрипта на исполнение интерпретатором Python. Проверка системного журнала. Чтобы примонтировать виртуальную файловую систему FUSE, необходимо выбрать ранее созданную точку монтирования, которая должна быть пустой, и выполнить команду от имени суперпользователя:

root@astra:/home/dytheadoct/mnt/fuse# `/home/dytheadoct/projects/fusexmp/fusexmp /home/dytheadoct/mnt/fuse/ -o allow_other`

При успешном монтировании в системном журнале можно увидеть информацию о выбранном режимах работы, о размере хэша, об успешной загрузке конфигурационного файла и ключа проверки.
...
Ключ проверки загружен

При попытке исполнить файл интерпретатором Python можно увидеть, что исполнение файла проходит успешно:

root@astra:/home/dytheadoct/apps# `python good.py`

а в системном журнале при этом никаких записей не появляется. Это обусловлено тем, что проверка целостности пройдена успешно.

3. Изменение файла скрипта. Запуск на исполнение интерпретатором Python. Проверка системного журнала. Теперь нужно изменить содержимое файла «good.py», например, дописав в него какую-либо информацию, тем самым будет нарушена целостность файла.После этого необходимо еще раз запустить на исполнение данный файл из примонтированной области

/home/mnt/fuse/home/apps# `python good.py`

При этом в системном журнале можно наблюдать запись об ошибке на этапе сравнения хэшей при доступе к файлу «good.py».Соответственно, проверка целостности пройдена и в ходе ее проведения было выявлено, что целостность данных была нарушена, поэтому доступ к файлу был запрещен. На этом демонстрация контрольного примера окончена.
